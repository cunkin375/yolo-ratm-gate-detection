...
def main():
    ...
    model.train(
        ...
        epochs=300,
        save_period=10,       # Save model every 25 epochs
        imgsz=640,
        batch=-1,             # Use Ultralytics auto batch feature
        optimizer="MuSGD",    # 2026 Hybrid optimizer
        augment=True,
        worksers=8,
        cache='ram',          # Speed up training on Windows SSDs
        amp=True,             # Use L4 tensor cores
        val=True
    )
    ...
    # Export to TensorRT for optimized NVIDIA deployment
